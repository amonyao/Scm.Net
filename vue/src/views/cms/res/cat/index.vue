		...
		<scSearch>
			<template #search>
				<el-form ref="formRef" label-width="100px" :model="param" :inline="true">
					<el-form-item label="类型" prop="types_id">
						<sc-select v-model="param.types_id" placeholder="类型" :data="types_list" />
					</el-form-item>
...
export default {
	...
	data() {
		return {
			tableName: 'cms_res_cat',
			apiObj: this.$API.cmsrescat.page,
			list: [],
			param: {
				types_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'namec', label: '名称', align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.cmsrescat.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.cmsrescat.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.cmsrescat.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.cmsrescat.delete, this.selection);
		},
		open_dialog(row) {
