...
export default {
	...
	data() {
		return {
			...
			column: [
				...
				{ prop: 'namec', label: '中文名称', minWidth: 100, align: 'left' },
				{ prop: 'namee', label: 'English Name', width: 160 },
	...
	methods: {
		...
		status_list(status) {
			this.$SCM.status_list(this, this.$API.cmsresorigin.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.cmsresorigin.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.cmsresorigin.delete, this.selection);
		},
		open_dialog(row) {
