			...
			<template #search>
				<el-form ref="formRef" label-width="100px" :model="param" :inline="true">
					<el-form-item label="国别" prop="nation_id">
						<sc-select v-model="param.nation_id" placeholder="请选择" :data="nation_list" />
					</el-form-item>
...
export default {
	...
	data() {
		return {
			tableName: 'cms_res_dynasty',
			apiObj: this.$API.cmsresdynasty.page,
			list: [],
			param: {
				nation_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'names', label: '简称', width: 100, align: 'left' },
				{ prop: 'namec', label: '全称', align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.cmsresdynasty.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.cmsresdynasty.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.cmsresdynasty.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.cmsresdynasty.delete, this.selection);
		},
		open_dialog(row) {
