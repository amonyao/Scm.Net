...
export default {
	...
	data() {
		return {
			tableName: 'cms_res_style',
			apiObj: this.$API.cmsresstyle.page,
			list: [],
			param: {
				option_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'types', label: '', width: 100 },
				{ prop: 'od', label: '', width: 100 },
				{ prop: 'title', label: '', width: 100 },
				{ prop: 'style', label: '', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.cmsresstyle.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.cmsresstyle.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.cmsresstyle.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.cmsresstyle.delete, this.selection);
		},
		open_dialog(row) {
