	...
	<sc-dialog v-model="visible" show-fullscreen destroy-on-close :title="titleMap[mode]" width="750px" @close="close">
		<el-form ref="formRef" label-width="100px" :model="formData" :rules="rules">
			...
			<el-form-item label="路径" prop="uri">
				<el-input v-model="formData.uri" placeholder="请输入路径" :maxlength="1024" show-word-limit clearable></el-input>
			</el-form-item>
			<el-form-item label="备注" prop="remark">
				<el-input v-model="formData.remark" placeholder="请输入备注" :maxlength="1024" show-word-limit
					clearable></el-input>
			</el-form-item>
		</el-form>
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === '0') {
						res = await this.$API.cmsfavuri.add.post(this.formData);
					} else {
						res = await this.$API.cmsfavuri.update.put(this.formData);
					}
