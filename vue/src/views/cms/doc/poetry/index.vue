...
export default {
	...
	data() {
		return {
			tableName: 'cms_doc_poetry',
			apiObj: this.$API.cmsdocpoetry.page,
			list: [],
			param: {
				types: 10,
				row_status: 1,
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.cmsdocpoetry.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.cmsdocpoetry.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.cmsdocpoetry.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.cmsdocpoetry.delete, this.selection);
		},
		open_dialog(row) {
