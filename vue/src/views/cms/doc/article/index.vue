	...
	<el-container class="is-vertical">
		<sc-search @search="search">
			...
			<template #filter>
				...
				<el-divider direction="vertical"></el-divider>
				<el-button icon="el-icon-plus" type="primary" @click="open_poetry()">新增诗词</el-button>
				<el-button icon="el-icon-plus" type="primary" @click="open_litera()">新增网文</el-button>
			</template>
		</sc-search>
		...
		<poetry ref="poetry" @complete="complete" />
		<litera ref="litera" @complete="complete" />
	</el-container>
</template>
...
export default {
	...
	data() {
		return {
			...
			column: [
				...
				{ prop: 'title', label: '主标题', minWidth: 240, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.cmsdocarticle.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.cmsdocarticle.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.cmsdocarticle.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.cmsdocarticle.delete, this.selection);
		},
		open_dialog(row) {
