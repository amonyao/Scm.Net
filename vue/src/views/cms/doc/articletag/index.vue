...
export default {
	...
	data() {
		return {
			apiObj: this.$API.cmsdocarticletag.page,
			list: [],
            param:{
				option_id: '0',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'article_id', label: '文章', width: 100 },
                { prop: 'tag_id', label: '标签', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.cmsdocarticletag.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.cmsdocarticletag.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.cmsdocarticletag.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.cmsdocarticletag.delete, this.selection);
		},
		open_dialog(row) {
