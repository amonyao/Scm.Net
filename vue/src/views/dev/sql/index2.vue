...
export default {
	...
	data() {
		return {
			...
			column: [
                ...
                { prop: 'sql', label: '内容', width: 100 },
                { prop: 'qty', label: '执行次数', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.devsql.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.devsql.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.devsql.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.devsql.delete, this.selection);
		},
		open_dialog(row) {
