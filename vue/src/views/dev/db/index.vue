...
export default {
	...
	data() {
		return {
			...
			column: [
				...
				{ prop: 'schame', label: '数据库', width: 100 },
	...
	mounted() {
		this.$SCM.list_dic(this.row_status_list, true);
	},
	methods: {
		complete() {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.devdb.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.devdb.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.devdb.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.devdb.delete, this.selection);
		},
		open_dialog(row) {
