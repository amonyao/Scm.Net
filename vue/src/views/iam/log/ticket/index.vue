...
export default {
	...
	data() {
		return {
			tableName: 'iam_log_ticket',
			apiObj: this.$API.iamlogticket.page,
			list: [],
			param: {
				option_id: '0',
				row_status: '1',
			...
			column: [
				...
				{ prop: 'osp_id', label: '', width: 100 },
				{ prop: 'ticket', label: '', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.iamlogticket.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.iamlogticket.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.iamlogticket.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.iamlogticket.delete, this.selection);
		},
		open_dialog(row) {
