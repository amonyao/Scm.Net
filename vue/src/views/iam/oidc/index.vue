...
export default {
	...
	data() {
		return {
			...
			column: [
				...
				{ prop: 'code', label: 'UnionID', width: 100 },
				{ prop: 'osp_id', label: '服务名称', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.iamoidc.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.iamoidc.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.iamoidc.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.iamoidc.delete, this.selection);
		},
		open_dialog(row) {
