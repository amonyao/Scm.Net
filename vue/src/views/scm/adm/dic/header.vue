...
export default {
	...
	methods: {
		...
		async initTree(param) {
			const t = await this.$API.scmadmdicheader.list.get(param);
			let _tree = [
		...
		async open(row) {
			...
			if (!row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.scmadmdicheader.model.get(row.id);
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === '0') {
						res = await this.$API.scmadmdicheader.add.post(this.formData);
					} else {
						res = await this.$API.scmadmdicheader.update.put(this.formData);
					}
