...
export default {
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.scmadmconfig.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.scmadmconfig.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.scmadmconfig.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.scmadmconfig.delete, this.selection);
		},
		open_dialog(row) {
			if (row.id) {
		...
		async getGroup(param) {
			...
			const res = await this.$API.scmadmconfigcat.list.get(param);
		...
		remove(node, data) {
			this.$confirm(`确定要删除选中的 ${data.label} 项吗？`, "提示", {
				...
				.then(async () => {
					...
					var res = await this.$API.scmadmconfigcat.delete.delete(
						data.id
					);
					if (res.code == 200) {
						this.columnComplete();
