...
export default {
	...
	methods: {
		...
		async initTotal() {
			var res = await this.$API.sysnotice.summary.get();
		...
		//设置/取消存档
		async setArchive(it) {
			let http = it.status == 0 ? this.$API.sysnotice.arced : this.$API.sysnotice.unarc;
		...
		async setSignRead(it) {
			let http = it.unread ? this.$API.sysnotice.readed : this.$API.sysnotice.unread;
