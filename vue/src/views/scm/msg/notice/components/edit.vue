<template>
	<div>
		...
		<div class="notice-main-title">
			<el-input v-model="formData.title" placeholder="通知主题" :maxlength="30" show-word-limit clearable
		...
		<div class="notice-context">
			<sc-editor v-model="formData.content" placeholder="请输入" :height="340"></sc-editor>
		</div>
		<div class="notice-btn">
			<el-button-group>
				<el-button icon="el-icon-position" @click="save(2)" :loading="isSaveing" type="primary">发送</el-button>
				<el-button icon="el-icon-finished" @click="save(1)" :loading="isSaveing" type="primary">存草稿</el-button>
			</el-button-group>
		</div>
...
export default {
	...
	data() {
		return {
			...
			uploadApi: this.$API.scmsysfile.notice,
			isOpenUser: false,
			erceipt: [],
			isSaveing: false,
		};
	...
	methods: {
		...
		async save(operate) {
			...
			this.isSaveing = true;
			this.formData.recipients = this.erceipt.map((item) => {
				return item.id;
			});
