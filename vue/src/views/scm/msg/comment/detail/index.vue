...
export default {
	...
	data() {
		return {
			apiObj: this.$API.scmmsgcommentdetail.page,
			list: [],
			param: {
				hid: '0',
				option_id: '',
			...
			column: [
				...
				{ prop: 'comment_id', label: '主题ID', width: 100 },
				{ prop: 'score', label: '评分', width: 100 },
				{ prop: 'comment', label: '评论', width: 100 },
				{ prop: 'rid', label: '回复ID', width: 100 },
				{ prop: 'pid', label: '引用ID', width: 100 },
	...
	methods: {
		async init() {
			...
			var res = await this.$API.scmmsgcommentdetail.page.get(this.param);
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.scmmsgcommentdetail.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.scmmsgcommentdetail.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.scmmsgcommentdetail.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.scmmsgcommentdetail.delete, this.selection);
		},
		show_search() {
