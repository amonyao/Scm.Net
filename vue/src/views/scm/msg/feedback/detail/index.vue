...
export default {
	...
	data() {
		return {
			apiObj: this.$API.msgfeedbackdetail.page,
			list: [],
			param: {
				option_id: '',
				row_status: '1',
			...
			column: [
				...
				{ prop: 'header_id', label: '反馈ID', width: 100 },
				{ prop: 'content', label: '回复内容', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.msgfeedbackdetail.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.msgfeedbackdetail.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.msgfeedbackdetail.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.msgfeedbackdetail.delete, this.selection);
		},
		open_dialog(row) {
