<template>
	<el-container>
		<el-aside width="260px" v-loading="showGrouploading">
			<el-container>
				<el-header>
					<sc-select v-model="param.types" @change="typeChange" :data="options">
					</sc-select>
				</el-header>
				...
				<el-scrollbar ref="scrollbar">
					...
					<div v-for="(item, index) in data" :key="index" class="sc-file-select__item"
						...
						<div class="sc-file-select__item__file">
							...
							<el-image v-if="_isImg(item.fileExt)" :src="serverApi + item.fileName"
								:preview-src-list="[serverApi + item.fileName]" fit="contain" lazy></el-image>
							<div v-else class="item-file item-file-doc">
								<el-icon class="el-icon--upload"><el-icon-document /></el-icon>
							</div>
						</div>
