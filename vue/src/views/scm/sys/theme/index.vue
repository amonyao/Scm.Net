...
export default {
	...
	data() {
		return {
			...
			column: [
				...
				{ prop: 'names', label: '主题名称', minWidth: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.scmsystheme.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.scmsystheme.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.scmsystheme.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.scmsystheme.delete, this.selection);
		},
		show_search() {
