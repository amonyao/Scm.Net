...
export default {
	...
	data() {
		return {
			...
			column: [
                ...
                { prop: 'names', label: '队列名称', width: 100 },
                { prop: 'remark', label: '队列说明', width: 100 },
                { prop: 'qty', label: '总数量', width: 100 },
                { prop: 'current', label: '当前号码', width: 100 },
                { prop: 'prefix', label: '号码前缀', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.scmqcsdetail.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.scmqcsdetail.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.scmqcsdetail.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.scmqcsdetail.delete, this.selection);
		},
		open_dialog(row) {
