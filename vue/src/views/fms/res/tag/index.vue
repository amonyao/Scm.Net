...
export default {
	...
	data() {
		return {
			apiObj: this.$API.fesrestag.page,
			list: [],
            param:{
				option_id: 0,
				row_status: 1,
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.fesrestag.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.fesrestag.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.fesrestag.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.fesrestag.delete, this.selection);
		},
		open_dialog(row) {
