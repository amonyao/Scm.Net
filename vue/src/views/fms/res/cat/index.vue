...
export default {
	...
	data() {
		return {
			tableName: 'fes-res-cat',
			apiObj: this.$API.fesrescat.page,
			list: [],
			param: {
				option_id: 0,
				row_status: 1,
			...
			column: [
				...
				{ prop: 'names', label: '类别名称', minWidth: 200 },
				{ prop: 'pid', label: '上级类别', width: 100 },
				{ prop: 'od', label: '显示排序', width: 100 },
				{ prop: 'qty', label: '图像数量', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.fesrescat.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.fesrescat.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.fesrescat.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.fesrescat.delete, this.selection);
		},
		open_dialog(row) {
