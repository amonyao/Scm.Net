...
export default {
	...
	data() {
		return {
			tableName: 'fes-res-color',
			apiObj: this.$API.fesrescolor.page,
			list: [],
			param: {
				option_id: 0,
				row_status: 1,
			...
			column: [
				...
				{ prop: 'color', label: '颜色', minWidth: 200, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.fesrescolor.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.fesrescolor.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.fesrescolor.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.fesrescolor.delete, this.selection);
		},
		open_dialog(row) {
