...
export default {
	...
	methods: {
		async initTenant() {
			const res = await this.$API.urunit.page.get({ limit: 1000 });
		...
		async getGroup(tenant) {
			...
			const res = await this.$API.urrole.tenantlist.get({
				tenantId: tenant,
			});
		...
		async initMenu() {
			...
			var resMenu = await this.$API.mgrmenu.list.get({ tenantId: 1 });
		...
		async groupClick(data) {
			...
			if (!data.children) {
				...
				const res = await this.$API.sysauthority.role.get(data.id);
				const that = this;
				if (res.code == 200) {
		...
		async saveAuthorize() {
			...
			this.loading = true;
			const res = await this.$API.sysauthority.plusRole.post(data);
			this.loading = false;
			if (res.code == 200) {
