...
export default {
	...
	methods: {
		...
		async getMenu() {
			...
			var res = await this.$API.mgrmenu.list.get({ tenantId: 1 });
		...
		async nodeDrop(draggingNode, dropNode, dropType) {
			...
			this.menuloading = true;
			var res = await this.$API.mgrmenu.dragging.put({
		...
		async add(node, data) {
			...
			this.menuloading = true;
			var res = await this.$API.mgrmenu.temp.post(newMenuData);
			this.menuloading = false;
		...
		async delMenu() {
			...
			var res = await this.$API.mgrmenu.delete.delete(
				reqData.ids.join(",")
			);
			this.menuloading = false;
