...
export default {
	...
	data() {
		return {
			apiObj: this.$API.omssoheader.page,
			list: [],
			param: {
				option_id: 0,
				row_status: 1,
			...
			column: [
				...
				{ prop: 'codes', label: '系统编码', width: 140 },
				{ prop: 'codec', label: '订单编号', width: 140, align: 'left' },
				{ prop: 'sale_amt', label: '订单金额', width: 100, align: 'right' },
				{ prop: 'paid_amt', label: '成交金额', width: 100, align: 'right' },
				{ prop: 'remark', label: '订单备注', minWidth: 160, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.omssoheader.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.omssoheader.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.omssoheader.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.omssoheader.delete, this.selection);
		},
		open_dialog(row) {
