...
export default {
	...
	data() {
		return {
			apiObj: this.$API.posresspuspec.page,
			list: [],
            param:{
				option_id: '',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'codec', label: '规格编码', width: 100 },
                { prop: 'names', label: '系统名称', width: 100 },
                { prop: 'namec', label: '规格名称', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.posresspuspec.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.posresspuspec.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.posresspuspec.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.posresspuspec.delete, this.selection);
		},
		open_dialog(row) {
