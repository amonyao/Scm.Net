...
export default {
	...
	data() {
		return {
			apiObj: this.$API.posresspuexts.page,
			list: [],
            param:{
				option_id: '',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'namec', label: '属性名称', width: 100 },
                { prop: 'json', label: '属性内容', width: 100 },
                ...
                { prop: 'creaet_time', label: '创建时间', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.posresspuexts.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.posresspuexts.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.posresspuexts.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.posresspuexts.delete, this.selection);
		},
		open_dialog(row) {
