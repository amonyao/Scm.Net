...
export default {
	...
	data() {
		return {
			apiObj: this.$API.possoorderheader.page,
			list: [],
            param:{
				option_id: '',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'codes', label: '', width: 100 },
                { prop: 'codec', label: '', width: 100 },
                { prop: 'item_need_qty', label: '', width: 100 },
                { prop: 'item_real_qty', label: '', width: 100 },
                { prop: 'unit_need_qty', label: '', width: 100 },
                { prop: 'unit_real_qty', label: '', width: 100 },
                { prop: 'handle', label: '', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.possoorderheader.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.possoorderheader.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.possoorderheader.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.possoorderheader.delete, this.selection);
		},
		open_dialog(row) {
