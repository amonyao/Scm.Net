...
export default {
	...
	data() {
		return {
			apiObj: this.$API.possoorderdetail.page,
			list: [],
            param:{
				option_id: '',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'order_id', label: '', width: 100 },
                { prop: 'od', label: '显示排序', width: 100 },
                { prop: 'spu_id', label: '商品ID', width: 100 },
                { prop: 'spec_id', label: '规格ID', width: 100 },
                { prop: 'exts', label: '扩展属性', width: 100 },
                { prop: 'need_qty', label: '需求数量', width: 100 },
                { prop: 'real_qty', label: '实际数量', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.possoorderdetail.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.possoorderdetail.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.possoorderdetail.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.possoorderdetail.delete, this.selection);
		},
		open_dialog(row) {
