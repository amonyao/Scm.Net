...
export default {
	...
	data() {
		return {
			apiObj: this.$API.possedetail.page,
			list: [],
            param:{
				option_id: '',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'se_id', label: '活动ID', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.possedetail.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.possedetail.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.possedetail.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.possedetail.delete, this.selection);
		},
		open_dialog(row) {
