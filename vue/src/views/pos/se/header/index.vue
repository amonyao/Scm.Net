...
export default {
	...
	data() {
		return {
			apiObj: this.$API.posseheader.page,
			list: [],
            param:{
				option_id: '',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'codec', label: '活动编码', width: 100 },
                { prop: 'names', label: '系统名称', width: 100 },
                { prop: 'namec', label: '活动名称', width: 100 },
                { prop: 'types', label: '活动类型', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.posseheader.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.posseheader.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.posseheader.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.posseheader.delete, this.selection);
		},
		open_dialog(row) {
