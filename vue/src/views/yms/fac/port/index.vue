...
export default {
	...
	data() {
		return {
			tableName: 'yms_fac_port',
			apiObj: this.$API.ymsfacport.page,
			list: [],
			param: {
				area_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'types', label: '大门类型', width: 100 },
				{ prop: 'modes', label: '出入模式', width: 100, align: 'left' },
				{ prop: 'codes', label: '系统代码', width: 100, align: 'left' },
				{ prop: 'codec', label: '大门编码', width: 100, align: 'left' },
				{ prop: 'names', label: '系统名称', width: 100, align: 'left' },
				{ prop: 'namec', label: '大门名称', minWidth: 100, align: 'left' },
	...
	mounted() {
		...
		this.$SCM.list_option(this.area_list, this.$API.ymsfacarea.option, {}, true);
	},
	methods: {
		complete() {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsfacport.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsfacport.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsfacport.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsfacport.delete, this.selection);
		},
		open_dialog(row) {
