<template>
	<sc-dialog v-model="visible" show-fullscreen destroy-on-close :title="titleMap[mode]" width="750px" @close="close">
		<el-form ref="formRef" label-width="100px" :model="formData" :rules="rules">
			...
			<el-form-item label="楼层编码" prop="codec">
				<el-input v-model="formData.codec" placeholder="请输入楼层编码" :maxlength="32" show-word-limit
					clearable></el-input>
			</el-form-item>
			<el-form-item label="楼层名称" prop="namec">
				<el-input v-model="formData.namec" placeholder="请输入楼层名称" :maxlength="32" show-word-limit
					clearable></el-input>
			</el-form-item>
	...
	data() {
		return {
			...
			rules: {
				...
				codec: [
					{ required: true, trigger: "blur", message: "楼层编码不能为空" },
				],
				namec: [
					{ required: true, trigger: "blur", message: "楼层名称不能为空" },
				],
			},
	...
	methods: {
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.ymsfacfloor.edit.get(row.id);
