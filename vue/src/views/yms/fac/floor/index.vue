...
export default {
	...
	data() {
		return {
			...
			column: [
				...
				{ prop: 'codec', label: '楼层编码', width: 100, align: 'left' },
				{ prop: 'namec', label: '楼层名称', minWidth: 100, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsfacfloor.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsfacfloor.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsfacfloor.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsfacfloor.delete, this.selection);
		},
		open_dialog(row) {
