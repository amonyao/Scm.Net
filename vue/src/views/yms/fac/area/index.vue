...
export default {
	...
	data() {
		return {
			...
			column: [
				...
				{ prop: 'codec', label: '园区编码', width: 100, align: 'left' },
				{ prop: 'names', label: '系统名称', width: 100, align: 'left' },
				{ prop: 'namec', label: '园区名称', minWidth: 100, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsfacarea.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsfacarea.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsfacarea.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsfacarea.delete, this.selection);
		},
		open_dialog(row) {
