...
export default {
	...
	data() {
		return {
			tableName: 'yms_fac_build',
			apiObj: this.$API.ymsfacbuild.page,
			list: [],
			param: {
				area_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'codec', label: '楼宇编码', width: 100, align: 'left' },
				{ prop: 'namec', label: '楼宇名称', minWidth: 100, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsfacbuild.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsfacbuild.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsfacbuild.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsfacbuild.delete, this.selection);
		},
		open_dialog(row) {
