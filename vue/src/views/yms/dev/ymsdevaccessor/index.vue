...
export default {
	...
	data() {
		return {
			apiObj: this.$API.ymsdevaccessor.page,
			list: [],
            param:{
				option_id: '0',
				row_status: '1',
			...
			column: [
                ...
                { prop: 'codec', label: '门禁编码', width: 100 },
                { prop: 'names', label: '系统名称', width: 100 },
                { prop: 'namec', label: '门禁名称', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsdevaccessor.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsdevaccessor.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsdevaccessor.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsdevaccessor.delete, this.selection);
		},
		open_dialog(row) {
