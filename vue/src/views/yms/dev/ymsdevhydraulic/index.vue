...
export default {
	...
	data() {
		return {
			apiObj: this.$API.ymsdevhydraulic.page,
			list: [],
            param:{
				option_id: '0',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'area_id', label: '所属园区', width: 100 },
                { prop: 'build_id', label: '所属楼宇', width: 100 },
                { prop: 'floor_id', label: '所属楼层', width: 100 },
                { prop: 'room_id', label: '所属房间', width: 100 },
                { prop: 'codes', label: '系统代码', width: 100 },
                { prop: 'codec', label: '水利编码', width: 100 },
                { prop: 'names', label: '系统名称', width: 100 },
                { prop: 'namec', label: '水利名称', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsdevhydraulic.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsdevhydraulic.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsdevhydraulic.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsdevhydraulic.delete, this.selection);
		},
		open_dialog(row) {
