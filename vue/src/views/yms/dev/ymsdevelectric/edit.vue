...
<el-form-item label="电力编码" prop="codec"> 
	<el-input 
...
<el-form-item label="电力名称" prop="namec"> 
	<el-input 
...
namec:'', 
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.ymsdevelectric.edit.get(row.id);
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === '0') {
						res = await this.$API.ymsdevelectric.add.post(this.formData);
					} else {
						res = await this.$API.ymsdevelectric.update.put(this.formData);
					}
