...
<el-form-item label="燃气编码" prop="codec"> 
	<el-input 
...
<el-form-item label="燃气名称" prop="namec"> 
	<el-input 
...
namec:'', 
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.ymsdevenergy.edit.get(row.id);
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === 0) {
						res = await this.$API.ymsdevenergy.add.post(this.formData);
					} else {
						res = await this.$API.ymsdevenergy.update.put(this.formData);
					}
