...
export default {
	...
	data() {
		return {
			apiObj: this.$API.ymsdevenergy.page,
			list: [],
            param:{
				option_id: '0',
				row_status: 1,
			...
			column: [
                ...
                { prop: 'codec', label: '燃气编码', width: 100 },
                { prop: 'names', label: '系统名称', width: 100 },
                { prop: 'namec', label: '燃气名称', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsdevenergy.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsdevenergy.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsdevenergy.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsdevenergy.delete, this.selection);
		},
		open_dialog(row) {
