...
<el-form-item label="通讯编码" prop="codec"> 
	<el-input 
...
<el-form-item label="通讯名称" prop="namec"> 
	<el-input 
...
namec:'', 
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.ymsdevnetwork.edit.get(row.id);
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === 0) {
						res = await this.$API.ymsdevnetwork.add.post(this.formData);
					} else {
						res = await this.$API.ymsdevnetwork.update.put(this.formData);
					}
