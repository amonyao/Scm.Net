<template>
	<sc-dialog v-model="visible" show-fullscreen destroy-on-close :title="titleMap[mode]" width="750px" @close="close">
		<el-form ref="formRef" label-width="100px" :model="formData" :rules="rules">
			...
			<el-form-item label="监控编码" prop="codec">
				<el-input v-model="formData.codec" placeholder="请输入监控编码" :maxlength="32" show-word-limit
					clearable></el-input>
			</el-form-item>
			<el-form-item label="监控名称" prop="namec">
				<el-input v-model="formData.namec" placeholder="请输入监控名称" :maxlength="32" show-word-limit
					clearable></el-input>
			</el-form-item>
		</el-form>
	...
	methods: {
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.ymsdevmonitor.edit.get(row.id);
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === '0') {
						res = await this.$API.ymsdevmonitor.add.post(this.formData);
					} else {
						res = await this.$API.ymsdevmonitor.update.put(this.formData);
					}
