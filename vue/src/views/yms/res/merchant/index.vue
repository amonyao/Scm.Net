...
export default {
	...
	data() {
		return {
			tableName: 'yms_res_merchant',
			apiObj: this.$API.ymsresmerchant.page,
			list: [],
			param: {
				option_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'codec', label: '商户编码', width: 100 },
				{ prop: 'names', label: '系统名称', width: 100 },
				{ prop: 'namec', label: '商户名称', minWidth: 100, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsresmerchant.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsresmerchant.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsresmerchant.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsresmerchant.delete, this.selection);
		},
		open_dialog(row) {
