...
export default {
	...
	data() {
		return {
			tableName: 'yms_res_supplier',
			apiObj: this.$API.ymsressupplier.page,
			list: [],
			param: {
				option_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'codec', label: '供应商编码', width: 100 },
				{ prop: 'names', label: '系统名称', width: 100 },
				{ prop: 'namec', label: '供应商名称', minWidth: 100, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsressupplier.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsressupplier.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsressupplier.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsressupplier.delete, this.selection);
		},
		open_dialog(row) {
