			...
			<template #search>
				<el-form ref="formRef" label-width="100px" :model="param" :inline="true">
					<el-form-item label="所属商户" prop="merchant_id">
						<sc-select v-model="param.merchant_id" placeholder="请选择" :data="merchant_list" />
					</el-form-item>
...
export default {
	...
	data() {
		return {
			tableName: 'yms_res_employee',
			apiObj: this.$API.ymsresemployee.page,
			list: [],
			param: {
				merchant_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'merchant_names', label: '所属商户', width: 100 },
				{ prop: 'codes', label: '系统代码', width: 100 },
				{ prop: 'codec', label: '租客编码', width: 100 },
				{ prop: 'names', label: '系统名称', width: 100 },
				{ prop: 'namec', label: '租客名称', minWidth: 100, align: 'left' },
			...
			merchant_list: [this.$SCM.OPTION_ALL],
		};
	},
	mounted() {
		this.$SCM.list_status(this.row_status_list, true);
		this.$SCM.list_option(this.merchant_list, this.$API.ymsresmerchant.option, {}, true);
	},
	methods: {
		complete() {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsresemployee.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsresemployee.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsresemployee.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsresemployee.delete, this.selection);
		},
		open_dialog(row) {
