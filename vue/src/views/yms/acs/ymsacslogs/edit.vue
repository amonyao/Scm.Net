<template>
	<sc-dialog v-model="visible" show-fullscreen destroy-on-close :title="titleMap[mode]" width="750px" @close="close">
		<el-form ref="formRef" label-width="100px" :model="formData" :rules="rules">
			<el-form-item label="入园类型" prop="types"> 
	<el-input 
...
driver:'', 
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.ymsacslogs.edit.get(row.id);
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === 0) {
						res = await this.$API.ymsacslogs.add.post(this.formData);
					} else {
						res = await this.$API.ymsacslogs.update.put(this.formData);
					}
