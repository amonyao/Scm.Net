...
export default {
	...
	data() {
		return {
			tableName: 'yms_acs_special',
			apiObj: this.$API.ymsacsspecial.page,
			list: [],
			param: {
				option_id: '0',
				row_status: 1,
			...
			column: [
				...
				{ prop: 'modes', label: '实体', width: 100 },
				{ prop: 'driver', label: '车辆信息', width: 100 },
				{ prop: 'reason', label: '原因', minWidth: 100, align: 'left' },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.ymsacsspecial.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.ymsacsspecial.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.ymsacsspecial.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.ymsacsspecial.delete, this.selection);
		},
		open_dialog(row) {
