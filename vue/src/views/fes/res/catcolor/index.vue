...
export default {
	...
	data() {
		return {
			apiObj: this.$API.fesrescatcolor.page,
			list: [],
            param:{
				option_id: 0,
				row_status: 1,
			...
			column: [
                ...
                { prop: 'color_id', label: '颜色', width: 100 },
                { prop: 'qty', label: '', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.fesrescatcolor.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.fesrescatcolor.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.fesrescatcolor.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.fesrescatcolor.delete, this.selection);
		},
		open_dialog(row) {
