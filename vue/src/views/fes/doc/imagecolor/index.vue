...
export default {
	...
	data() {
		return {
			apiObj: this.$API.fesdocimagecolor.page,
			list: [],
            param:{
				option_id: 0,
				row_status: 1,
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.fesdocimagecolor.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.fesdocimagecolor.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.fesdocimagecolor.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.fesdocimagecolor.delete, this.selection);
		},
		open_dialog(row) {
