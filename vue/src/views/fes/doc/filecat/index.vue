...
export default {
	...
	data() {
		return {
			apiObj: this.$API.fesdocfilecat.page,
			list: [],
            param:{
				option_id: 0,
				row_status: 1,
			...
			column: [
                ...
                { prop: 'cat_id', label: '类别', width: 100 },
                { prop: 'file_id', label: '图像', width: 100 },
                { prop: 'salt', label: '', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.fesdocfilecat.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.fesdocfilecat.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.fesdocfilecat.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.fesdocfilecat.delete, this.selection);
		},
		open_dialog(row) {
