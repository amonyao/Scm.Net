...
<template>
	<div class="scTable" :style="{ height: _height }" ref="scTableMain" v-loading="loading">
		...
		<div class="scTable-page">
			...
			<div class="scTable-do" v-if="!hideDo">
				...
				<el-popover v-if="!hideExcel" placement="top" ref="excelPover" title="导出Excel设置" :width="400"
					...
					<template #reference>
						<el-button icon="sc-icon-file-excel" circle style="margin-left: 15px"></el-button>
					</template>
		...
		<sc-contextmenu ref="contextmenu" @command="handleCommand" @visible-change="visibleChange">
			<sc-contextmenu-item v-if="menuDefault.includes('add')" command="add" title="添加" icon="el-icon-plus"
				suffix="Add"></sc-contextmenu-item>
			<sc-contextmenu-item v-if="menuDefault.includes('edit')" command="edit" title="编辑" icon="el-icon-edit"
				suffix="Uelete"></sc-contextmenu-item>
			<sc-contextmenu-item v-if="menuDefault.includes('delete')" command="delete" title="删除" icon="el-icon-delete"
				suffix="Delete"></sc-contextmenu-item>
			<sc-contextmenu-item v-for="(item, index) in menuColumn" :key="index" :command="item.command"
				:title="item.title" :icon="item.icon" :suffix="item.suffix"></sc-contextmenu-item>
			<sc-contextmenu-item command="refresh" title="重新加载(R)" icon="el-icon-refresh" divided></sc-contextmenu-item>
			<sc-contextmenu-item command="close" title="关闭" divided></sc-contextmenu-item>
		</sc-contextmenu>
	</div>
</template>
...
export default {
	...
	props: {
		...
		indent: { type: [String, Number], default: "50" },
