<template>
    <sc-dialog v-model="visible" show-fullscreen destroy-on-close :title="titleMap[mode]" width="560px" @close="close">
        <el-form ref="formRef" label-width="100px" :model="formData" :rules="rules">
            ...
            <el-form-item label="显示排序" prop="od">
                <el-input-number v-model="formData.od" placeholder="请输入显示排序" :maxlength="11" show-word-limit
...
export default {
    ...
    methods: {
        ...
        async open(row) {
            if (!row || !row.id) {
                this.mode = "add";
                this.formData.app = row.app;
                this.initTree(row.app, 0);
            } else {
                this.mode = "edit";
