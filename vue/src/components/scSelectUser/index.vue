<template>
	<div class="sel-user" v-if="!hideInput">
		<div class="sel-input">
			<el-tag v-for="(it, index) in resUser" :key="index" type="info" closable :disable-transitions="false"
				@close="delResUser(it)">
				{{ it.namec }}
			</el-tag>
	...
	<el-dialog ref="dialog" :model-value="selectOpen" title="选择用户" :width="width" v-bind="$attrs" :show-close="true">
		<div class="select-user">
			<div class="org-user">
				...
				<div class="org-user-box">
					...
					<div class="user">
						<el-scrollbar height="350px">
							<div class="item" v-for="(it, index) in user.items" :key="index">
								<el-checkbox v-model="it.checked" :disabled="it.disabled"
									@change="userChange($event, it)">{{ it.namec }}</el-checkbox>
							</div>
						</el-scrollbar>
					</div>
				</div>
			...
			<div class="select-res">
				...
				<div class="select-list">
					<el-scrollbar height="350px">
						<div class="item" v-for="(it, index) in resUser" :key="index">
							<div class="name">
								<span>{{
									it.namec.charAt(0).toUpperCase()
								}}</span>{{ it.namec }}
							</div>
